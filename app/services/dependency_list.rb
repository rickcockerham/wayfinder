...
class DependencyList
  def initialize(items, importance_map: nil)
    @items = items.to_a
    @imp   = importance_map # {id=>score} optional
  end
  ...
  def call
    ...
    edges = Hash.new { |h,k| h[k] = [] } # blocker -> [blocked Items]
    rev   = Hash.new { |h,k| h[k] = [] } # blocked -> [blocker Items]
    ...
    score = ->(it) { (@imp && @imp[it.id]) || safe_importance(it) }

    roots = @items.select { |it| rev[it.id].blank? }
    roots.sort_by! { |it| -score.call(it) }
    ...
    dfs = lambda do |node|
      ...
      order << node
      kids = (edges[node.id] || []).sort_by { |c| -score.call(c) }
      kids.each { |c| dfs.call(c) }
    end
